...
exp.getPodcastsByCategoryId = RouterAsyncErrorHandler(
  async (req, res, next) => {
    try {
...
exp.addPodcast = RouterAsyncErrorHandler(async (req, res, next) => {
  const { authorId, name, description, categoryId } = req.body;
  const isApproved = 0;
